...
快照)。2026-08 租户化后 _live 的键是 (租户, conv),personal 模式恒 ("local", conv)。但 send_image 是在回合进行中途触发的,后面还会有更多正文/工具事件——
