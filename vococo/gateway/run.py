...
class GatewayRunner:
    ...
    async def _handle(self, adapter: Adapter, inc: Incoming) -> None:
        key = inc.session_key
        # 人工续聊链路:用户手动给这个会话发消息 = 重新激活它(归档只该滤掉静默
        # 会话)——自动取消归档,任何会话(web: 普通对话 / task: 后台任务)一视同仁,
    ...
    def _install_loop_exception_handler(self) -> None:
        ...
        vococo.out.log 里只有 CLI 子进程自己吐的 JS 噪音,从没见过我们 Python
