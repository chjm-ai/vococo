...
class _WebSink(Sink):
    ...
    async def done(self, reply: AgentReply, turn_id: int | None = None) -> None:
        ...
        title = "主会话" if self.conv == "main" else (
            session_store.get_title(key) or config.PERSONA_NAME
        )
        self.a._push_notify(
            title=title,
...
def _document_ref(value: object) -> str | None:
    """规范化一条候选路径/URL；不是文档或只有代码后缀时返回 None。"""
    if not isinstance(value, str):
        return None
    value = value.strip().rstrip(_SESSION_DOC_TRAILING)
    if not value:
        return None
    # URL 查询参数/锚点不属于文件名；本地路径中的 # 也无需作为预览目标的一部分。
    filename = value.split("?", 1)[0].split("#", 1)[0]
    if Path(filename).suffix.lower() not in _SESSION_DOC_EXTENSIONS:
        return None
    return value
...
class WebAdapter:
    ...
    async def send(self, chat_id: int | str, text: str) -> None:
        ...
        self._push_notify(
            title="⚠️ 出错了" if is_err else config.PERSONA_NAME,
            body=text,
            conv=str(chat_id),
        ...
        return web.json_response(
            {
                ...
                "mcp": {
                    "vococo_enabled": settings_store.vococo_enabled(),
    ...
    def _fuzzy_doc_path(self, base: Path, rel_parts: tuple[str, ...]) -> Path | None:
        ...
        典型场景是 AI 提到项目文件时把包名前缀说漏了(比如把 vococo/memory/images.py
    ...
    # ── 发布页(公开可取,给 skill vococo-web-publish 用)────────────────────
