...
class RichSink(core.Sink):
    ...
    def _render(self):
        ...
        if self.answer:
            parts.append(
                Panel(Markdown(self.answer), title=config.PERSONA_NAME, border_style="green",
                      title_align="left")
            )
        elif not parts:
            parts.append(Text(f"{config.PERSONA_NAME} 思考中…", style="cyan"))
        return Group(*parts)
