...
PROMPT_LOAD_TIMEOUT: float = float(os.environ.get("PROMPT_LOAD_TIMEOUT", "1.5"))
# iCloud 读取一次超时后，短期内通常不会自行恢复。此窗口直接跳过，避免每个新回合
# 都重复白等 prompt 加载超时。
PROMPT_LOAD_BACKOFF: float = float(os.environ.get("PROMPT_LOAD_BACKOFF", "300"))
