...
个人 AI 助理(单用户),常驻进程 + 多入口(Web / CLI),跨入口共享会话与记忆。近期扩张为也能从 Web 入口、在选定项目里执行中等编码任务(个人助理 + 编码代理,同一套统一会话,不分模式)。本文件是领域术语表,只定义概念,不含实现细节。
...
**Approval Gate(审批闸)**:
对 `escalate` 的操作,在**有交互通道时**(Web)弹「允许一次 / 拒绝」请用户拍板;**无通道时**(CLI/eval/cron)放行(信任该通道);超时或发送失败视为拒绝。这是「远程编码:动手前对齐」的安全阀。
