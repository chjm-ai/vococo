...
> 一个常驻进程,能从 **Web / Telegram / 命令行**任意入口找它说话;它**记得我是谁、
...
3. **单进程常驻 + 多入口**:一个 gateway 进程同时监听 Web / Telegram,外加 CLI 直连;跨入口共享同一份会话与记忆。
...
| **Adapter(平台适配器)** | 每个入口一个适配器(Web / Telegram / CLI),把平台消息归一成统一格式 |
...
| P1-3 | **主动触达 / 定时汇报** | cron 定时任务,结果投递到 Web/TG |
...
```
入口层 Adapters:  Web / Telegram / CLI  (各一适配器,归一为统一消息格式)
        │
        ▼
...
| 默认模型 | `claude-opus-4-8`(可配) | 个人用要好脑子;注意周限额 |
