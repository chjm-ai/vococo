...
3. 屏幕先显示我说的话的文字(转写回显),接着 AI 的文字逐句浮现,同时语音播出来,
   回答只有一两句;
4. AI 说话时我可以点"■ 停"立刻闭嘴;
5. 我说"帮我查一下笔记里关于 XX 的内容",AI 先出声"稍等,我看看",查完之后把
   查到的要点**用语音完整讲出来**(不是只说一句"查到了"就把内容丢给屏幕)——
...
| F10 | 干活垫话(2026-07-07 补) | 本轮第一次出现顶层工具调用(查资料/跑命令等)时,立即插播一句预合成的固定短语(如"稍等,我看看"),不等模型自己说;和正式回复共用同一条播放队列 |
...
5. 预计要花一点时间才能干完的事(查资料、跑命令),后端会自动垫一句"稍等"
