...
# 的美元花费落在 $36~93。最早拿 claude-monitor 社区维护的 Max5 固定表
# cost_limit=$35 当锚定分母,上线后跟官方客户端实时对比了一次(同一时刻:
# 本地 $33.43 / 官方 69%),反推真实上限约 $48.44——这是"同一时刻两边都有
# 读数"的直接校准,比历史日志反推准(历史那 4 个点有窗口对齐误差),改用这个。
# 这依然不是官方精确值,只有 1 个实时校准样本,订阅档位变了、或者哪天差距
# 又变大了,要拿官方客户端当场对比重新校一次。
_FIVE_HOUR_COST_LIMIT_USD = 48.44
