...
claude-hermes 自己的 settings_store(web_providers)中。

用法:
  python -m claude_hermes.gateway.migrate_cc_switch [--dry-run]

--dry-run:只打印会导入什么,不写入 web_settings.json。

注意:此脚本只负责把 cc-switch 的数据搬到我们自己的存储。搬完后 claude-hermes
...
def _load_yaml() -> dict:
    """从 hermes_config_path() 读 YAML,尽量复用 providers 已有的 path 定义。"""
    cfg = providers.hermes_config_path()
    if not cfg.exists():
        return {}
    try:
        import yaml
